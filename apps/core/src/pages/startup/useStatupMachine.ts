...
import { interpret, StateFrom, Sender } from "xstate";
...
type MachineState = Pick<StateFrom<typeof startupMachine>, 'matches' | 'context'>;

export const useStatupMachine = () => {
  const [actor] = useState(interpret(startupMachine.withConfig({
    services: {
