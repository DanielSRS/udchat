...
export const saveOrgToStorageService = (
  context: ContextFrom<typeof orgMachine>,
) => {
  return new Promise((resolve, reject) => {
    saveOrganization(context.organization).then(val => {
      if (isLeft(val)) {
...
export const deleteOrgFromStorageService = (
  context: ContextFrom<typeof orgMachine>,
) => {
  return new Promise((resolve, reject) => {
    deleteOrganization(context.organization).then(val => {
      if (isLeft(val)) {
        return reject(val.left);
      }
      resolve(true);
